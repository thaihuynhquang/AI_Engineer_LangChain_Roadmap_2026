...
│   │   ├── online_learning_guide.md# Detailed free online course roadmap covering 5 Modules
│   │   ├── resources.md            # Directory of free learning resources, tools, and reference links
│   │   ├── schedule.md             # Sample study schedule & daily/weekly Pomodoro time allocation
│   │   └── tech_stack.md           # Analysis of 5 AI Engineer tech stack layers for 2026
...
│   │   └── planData.ts             # Data model containing 100% roadmap content (Sprints, Schedule, Resources, Tech Stack)
...
│   │   └── appState.ts             # Application-wide interfaces (Task, Module, Schedule, AppState, Logs...)
...
- **`online_learning_guide.md`**: Detailed breakdown of 5 online learning modules, recommended video courses, and hands-on exercises.
- **`resources.md`**: Catalog of free learning tools, cheat sheets, repositories, and documentation links.
- **`schedule.md`**: 4-week sample study schedule allocating Pomodoro sessions per day.
- **`tech_stack.md`**: Deep dive into the 5 AI Engineer tech stack layers for 2026 (Model Layer, Agent Framework, RAG & Vector DB, LLMOps, Client Integration).
...
#### `src/data/`
- **`planData.ts`**: Single Source of Truth containing 100% of static business data, including Sprint Modules (`SPRINT_MODULES`), Pomodoro Schedule (`POMODORO_SCHEDULE`), Resources (`FREE_RESOURCES`), and Tech Stack (`TECH_STACK_LAYERS`).
...
#### `src/views/`
Contains 5 Native Custom Elements (Light-DOM Web Components) representing the 5 main UI tabs:
...
- **`roadmap-view-roadmap.ts`**: Detailed 5-Sprint roadmap showing deliverables checklist and learning goals.
- **`roadmap-view-schedule.ts`**: Interactive Pomodoro timer with daily/weekly study schedule views.
- **`roadmap-view-resources.ts`**: Learning resource catalog supporting search and module filtering.
- **`roadmap-view-techstack.ts`**: Visual breakdown of the 2026 AI Engineer tech stack layers.
...
- **`router.ts`**: Hash router managing location hash changes (`#/dashboard`, `#/roadmap`, `#/schedule`, `#/resources`, `#/techstack`) and state synchronization.
